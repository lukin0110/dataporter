...
- **Neither the ask nor `extract-skills` can run headless against claude.ai.** Measured
  2026-09-15 on a signed-in profile, Chrome 152: `--headless=new` is served a Cloudflare interstitial at the export
  ...
  that needs a Claude export needs a display. `extract-skills` (`66`) stands on the same
  page to make its two reads, so the same interstitial stops it, and a read that fails
  headless names the flag the same way. Whether the **fetch** is refused the same way
