...
**Rehearsal run:** 2026-09-13. **Mode:** `non-interactive`.
**Tool:** dataporter 0.1.0. **Scripted agent:** scripted agent 1.0.0. **Chrome:** Google Chrome 152.0.7977.84.
**Mock:** 0.1.0.
...
| --- | --- | --- | --- |
| `setup` | 0 | 4 |  |
| `doctor (before login)` | 6 | 3.8 | a signed-out tab has redirected to /login |
| `login` | 0 | 3.3 |  |
| `doctor (after login)` | 0 | 4.2 |  |
| `import --dry-run` | 0 | 0.2 |  |
| `import --pilot` | 0 | 47.2 |  |
| `report (after pilot)` | 0 | 0.2 |  |
| `import --all (interrupted)` | -9 | 4.1 | SIGKILL mid-conversation |
| `import --all (again)` | 0 | 22 |  |
| `verify` | 0 | 2.7 |  |
| `report` | 0 | 0.2 |  |
| `followup` | 0 | 49.1 |  |
| `status` | 0 | 0.2 |  |
| `session status` | 0 | 2.1 |  |
| `sign_off.py gate` | 1 | 0.2 | its last rows are for a person to judge |
| `sign_off.py drill` | 0 | 0.2 |  |
| `sign_off.py safety` | 1 | 0.2 | its last rows are for a person to judge |
...
| --- | --- | --- | --- | --- |
| `setup` | — | — | — | — |
| `doctor (before login)` | `traces/02-doctor--before-login.jsonl` | 3 | — | `hermes 1.0.0 (scripted agent)` |
| `login` | `traces/03-login.jsonl` | 19 | `claude.ai` | `hermes 1.0.0 (scripted agent)` |
| `doctor (after login)` | `traces/04-doctor--after-login.jsonl` | 6 | — | `hermes 1.0.0 (scripted agent)` |
| `import --dry-run` | — | — | — | — |
| `import --pilot` | `traces/06-import---pilot.jsonl` | 293 | `claude.ai` | `hermes 1.0.0 (scripted agent)` |
| `report (after pilot)` | — | — | — | — |
| `import --all (interrupted)` | `traces/08-import---all--interrupted.jsonl` | 17 | `claude.ai` | `hermes 1.0.0 (scripted agent)` |
| `import --all (again)` | `traces/09-import---all--again.jsonl` | 126 | `claude.ai` | `hermes 1.0.0 (scripted agent)` |
| `verify` | `traces/10-verify.jsonl` | 55 | `claude.ai` | `hermes 1.0.0 (scripted agent)` |
| `report` | — | — | — | — |
| `followup` | `traces/12-followup.jsonl` | 292 | `claude.ai` | `hermes 1.0.0 (scripted agent)` |
| `status` | — | — | — | — |
| `session status` | — | — | — | — |
| `sign_off.py gate` | — | — | — | — |
| `sign_off.py drill` | — | — | — | — |
...
| --- | --- | --- | --- |
| every migratable conversation completed at its first attempt | 9/9, drill interrupted 1 | pass | *measured on 2026-09-13* |
| no pause recorded | human interventions: 0 | pass | *measured on 2026-09-13* |
| the report reconciles | 9 + 0 + 1 + 0 == 10 | pass | *measured on 2026-09-13* |
| verify finds the source line and every ack line | exit 0 | pass | *measured on 2026-09-13* |
| every conversation ends with its title set | 9/9 titles set | pass | *measured on 2026-09-13* |
| the drill loses no conversation and creates no duplicate chat | Interruption drill | pass | *measured on 2026-09-13* |
| the safety audit finds no host but claude.ai | other hosts: 0, chats this workspace did not create: 0 (other claude.ai paths: 1, for a person to judge) | pass | *measured on 2026-09-13* |
| ledger: chats created == distinct chat ids in the state | 10 == 9 + 1 (the chat the killed run created and never recorded) | pass | *measured on 2026-09-13* |
| ledger: messages received == parts sent | 11 == 10 + 1 (the part the killed run had sent) | pass | *measured on 2026-09-13* |
| ledger: files accepted == attachments migrated by upload | 1 == 1 | pass | *measured on 2026-09-13* |
| ledger: renames == titles set | 9 == 9 | pass | *measured on 2026-09-13* |
| ledger: sign-ins == logins + automatic sign-ins | 1 == 1 + 0 | pass | *measured on 2026-09-13* |

**Verdict:** passed. *measured on 2026-09-13*
