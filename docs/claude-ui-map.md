...
**Second reading:** 2026-09-18, one page read by hand — the sign-in screen served *at the
export page's own address*, kept trimmed as
[`spike/claude-sign-in-screen-2026-09-18.html`](spike/claude-sign-in-screen-2026-09-18.html).
It answers the `sign-in form` row and adds `sign-in screen in place` (`70`); no Hermes task
was run, and no other row changed.
...
| `sign-in form` (`24`, `50`) | `login_form.LOGIN_FIELDS_JS`: a visible `input[type="email"], input[autocomplete="username"]` for the email step; the password step `24` wrote — `input[type="password"], input[autocomplete="current-password"]` — is kept for a source that has one, and claude.ai has none (brief 07 §72): what follows the address is the `link sent` row below, which `login_form` reports as `code_or_challenge` and types nothing into. The agent's half (`signin.prompt`) is told to take the email path and never Google, Apple, SSO or a passkey, by looking; no source uses it since `52` | `docs/spike/claude-sign-in-screen-2026-09-18.html`: the first step, read at the export page's own address — two `button[data-testid="login-with-google"]` and `[data-testid="login-with-apple"]`, and a `form` holding `input[data-testid="email"][autocomplete="email"][type="email"]` and `button[type="submit"][data-testid="continue"]`. Read in Spanish, as the 2026-09-15 artefact was: none of it is a word. What `login_form` looks for — an `input[type="email"]` — is there, so the row is answered; the selector itself is still what `24` shipped | *observed on 2026-09-18* |
| `sign-in screen in place` (`70`) | `claude.SIGN_IN_ATTESTATION_SELECTOR` and `claude.SIGN_IN_FORM_SELECTOR`, both required, read in the page by `probe`'s `sign_in_showing` and turned into an answer by `export_page.signed_out`: `[data-client-attestation-channel="send_magic_link"]` and `form:has([data-testid="email"]):has([data-testid="continue"])`. Matched by presence and not by visibility — the attestation container is `aria-hidden` and sized to zero | `docs/spike/claude-sign-in-screen-2026-09-18.html`: a signed-out request for `/new#settings/data-privacy-controls` is **not** redirected. The application renders its sign-in there, so `location.pathname` is still `/new`, `kind_of` answers `NEW_CHAT`, and the `signed out` row above never fires. The head says what the body is — `<title>Sign in - Claude</title>` and a `meta[name="x-route-title"]` with `data-intended-title` — but both are English on a page whose every visible word is Spanish, so neither is read. The attestation container is the same one `claude-sign-in-link-sent.html` carried on 2026-09-15, which is the second page it has been seen on; the form is seen here only. Before `70` this state cost the ask `timeouts.ask_s` and exit `1` | *observed on 2026-09-18* |
