...
| `export button` (`51`) | a visible `[data-perf-screen="data-privacy-controls"] [data-settings-row] button[data-cds="Button"]`, clicked with `element.click()` on the first visible match. Matched by position because the panel offers nothing better: no test id, the rows' ids are React's and regenerated each render, and the only thing distinguishing Export from the five *Manage* rows under it is its words. Every row above it holds a switch, so the Export button is the panel's first | as above: the code is on the reading, the row waits on the evidence | *unknown* |
| `export confirmation` (`31`, `52`) | the `Export data` row does not ask for anything: it opens a **second screen** inside the same settings dialog, carrying a description, a `Conversations from` period, a list of what the export will include, and the button that asks — a visible `[data-testid="export-confirm-button"]`. `view.dialog` is true throughout, because the settings panel is itself a `[role="dialog"]` | a person read the screen's markup on a real account; no artefact under `spike/`, as above | *unknown* |
...
| `export requested` (`31`, `51`) | a visible `[data-testid="export-requested"]`, a placeholder that matches nothing on the real site. `31`'s `[role="status"]` fallback is **removed**: claude.ai carries several such live regions (screen-reader announcers, the toast region, a composer hint), `sr-only` clips rather than hides them so they read as visible, and on 2026-09-14 one matched before anything had been asked for — the run clicked the row, never reached `Export`, and wrote an `ask.json` for a request the vendor never received. An element that is there, never the sentence it holds — and the one signal that makes the ask write `ask.json` | the false positive, seen on a real account | *unknown* |
