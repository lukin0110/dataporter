#!/usr/bin/env node
// Graft's Claude Code statusline shim — renamed out of graft's way on purpose.
//
// Same reason as its sibling `repo-graft-hooks.cjs`, which carries the full explanation: the
// wiring refresh in `dist/upkeep.js:240` rewrites both shims together, and it only recognises
// this repo when `.claude/helpers/graft-hooks.cjs` exists (`dist/upkeep.js:211`). Renaming the
// pair is what makes that check fail; they are named alike so neither gets restored on its own.
//
