...
# own location for a hand-run from anywhere else.
cd "${CLAUDE_PROJECT_DIR:-${ORCA_WORKTREE_PATH:-$(dirname "$0")/../..}}" || exit 0
...
if command -v graft >/dev/null 2>&1; then
  graft build . || echo "ensure-graph: \`graft build\` failed; the graph is missing, run it yourself" >&2
elif command -v npx >/dev/null 2>&1; then
  npx -y @nanonets/graft build . || echo "ensure-graph: \`npx -y @nanonets/graft build\` failed; the graph is missing" >&2
else
