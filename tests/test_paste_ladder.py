...
def test_a_sample_ends_on_a_real_character() -> None:
    """`normalise` strips trailing whitespace.

    A sample ending in a half-written blank line would measure the comparison, not the
    composer.
    """
    text = paste_ladder.sample(5_000)
...
def test_a_sample_carries_the_shapes_the_ladder_is_about() -> None:
    """A leading space and a blank line between turns are the two things `normalise` and `blockText` disagree about."""
    text = paste_ladder.sample(3_000)
    assert "\n " in text
    assert "\n\n" in text
...
def fake_command(tmp_path: Path, body: str) -> str:
    """Return an executable printing `body` on stdout, for `helper` to read."""
...
def test_a_non_zero_exit_is_read_rather_than_raised(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def scripted(monkeypatch: pytest.MonkeyPatch, probe: dict[str, Any], paste: dict[str, Any]) -> list[str]:
    """Replace `helper` with canned answers; return the questions asked."""
    asked: list[str] = []

    def fake_helper(command: str, workspace: Path, *args: str) -> dict[str, Any]:
...
def test_a_round_with_no_composer_is_skipped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    asked = scripted(monkeypatch, {"ok": True, "composer_present": False}, {})
    row = paste_ladder.round_trip("cli", tmp_path, write_seed(tmp_path), "insert_text", prompt=True)
    assert "skipped" in row
    assert "inserted" not in row
    assert asked == []


def test_a_verbatim_round_is_measured(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    asked = scripted(monkeypatch, EMPTY_COMPOSER, {"ok": True, "chars": 4_000, "elapsed_ms": 12})
    row = paste_ladder.round_trip("cli", tmp_path, write_seed(tmp_path), "insert_text", prompt=True)
...
def test_a_text_mismatch_still_counts_as_an_insert(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """`paste` answering `text_mismatch` is the more interesting Q3 finding.

    The text reached the composer and came back changed.
    """
    asked = scripted(
        monkeypatch,
        EMPTY_COMPOSER,
...
@pytest.mark.parametrize("error", ["composer_not_empty", "composer_missing", "no_claude_tab"])
def test_a_refused_round_is_not_an_observation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, error: str) -> None:
    """`c7f8194`: a round that never inserted anything is not a round to ask about.

    Asking a human whether a chip appeared files their guess as an observation.
    """
...
def test_an_inserted_round_reports_whether_it_survived(verbatim: bool, expected: str) -> None:
...
def test_only_an_inserted_row_is_marked_observed() -> None:
    """`tests/test_spike_docs.py` reads these marks.

    A row marked `observed` for a round that inserted nothing would tell it the spike
    had run.
    """
    rows = [
...
def ladder_on_path(
    ...
    ladder_cli.write_shim(directory, cdp_port=session.client.port, server_port=server.port)
...
def test_the_ladder_measures_a_real_composer(
    ...
    """The whole instrument, end to end.

    Seed file, subprocess, CDP, composer, read-back, table. Only the host is not
    claude.ai.
    """
...
def test_without_a_human_only_the_first_round_is_measured(
    ...
    """Nothing in `08` clears a composer.

    Unattended the second round meets the first round's text. The rows stay honest and
    the warning says so.
    """
    ladder_on_path(live, tmp_path, monkeypatch)
    ...
    first, second = json.loads((notes.parent / "paste-ladder.json").read_text("utf-8"))["rows"]
...
def test_the_ladder_writes_nothing_into_the_repository(
    ...
    """`spike.NOTES` points inside the repository.

    A run that forgot to redirect would commit a fabricated observation into
    `docs/spike/`.
    """
    ladder_on_path(live, tmp_path, monkeypatch)
