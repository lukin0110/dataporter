...
def settings_for(attachments_dir: Path, **overrides: object) -> Settings:
    """Return Settings that look at `attachments_dir` and nothing else machine-specific."""
    return Settings(
...
def plan_of(export_dir: Path, attachments_dir: Path, **overrides: object) -> planning.MigrationPlan:
    return planning.build_plan(load_export(export_dir), settings_for(attachments_dir, **overrides))
...
def test_the_fixture_has_exactly_one_unsupported_conversation(export_dir: Path, attachments_dir: Path) -> None:
...
def test_the_branch_conversation_is_migratable_with_one_branch_dropped(export_dir: Path, attachments_dir: Path) -> None:
...
def test_an_attachment_with_extracted_content_is_inline(export_dir: Path, attachments_dir: Path) -> None:
    attachments = find(plan_of(export_dir, attachments_dir), ATTACHMENT_CONVERSATION).attachments
...
def test_a_file_with_no_bytes_is_unsupported(export_dir: Path, attachments_dir: Path) -> None:
    attachments = find(plan_of(export_dir, attachments_dir), ATTACHMENT_CONVERSATION).attachments
...
def test_dropping_the_bytes_in_flips_the_file_to_upload(export_dir: Path, attachments_dir: Path) -> None:
...
def test_bytes_beside_the_directory_are_found_too(export_dir: Path, attachments_dir: Path) -> None:
...
def test_the_same_file_in_files_and_files_v2_is_one_attachment(export_dir: Path, attachments_dir: Path) -> None:
...
def test_two_plans_of_one_export_serialise_identically(export_dir: Path, attachments_dir: Path) -> None:
    first = plan_of(export_dir, attachments_dir)
    second = plan_of(export_dir, attachments_dir)
    assert first.model_dump_json() == second.model_dump_json()


def test_the_plan_carries_the_export_fingerprint(export_dir: Path, attachments_dir: Path) -> None:
...
def test_migratable_and_unsupported_account_for_every_conversation(export_dir: Path, attachments_dir: Path) -> None:
...
def test_a_conversation_of_placeholders_has_no_representable_text(
    ...
    export = one_conversation(message(content=[{"type": "tool_result", "content": "created"}]))
...
def test_a_seed_over_the_hard_cap_is_not_migrated(attachments_dir: Path) -> None:
    ...
    capped = settings.model_copy(update={"seed": SeedSettings(max_chars=1_000, hard_max_chars=2_000)})
...
def test_an_empty_conversation_reports_no_seed_at_all(export_dir: Path, attachments_dir: Path) -> None:
...
@pytest.mark.parametrize("file_name", UNSUPPORTED_TYPES, ids=str)
...
def test_uploads_past_the_per_chat_cap_are_unsupported_in_document_order(
    ...
    export = one_conversation(message(text="hi", files=[{"file_name": f"f{index}.png"} for index in range(4)]))
...
@pytest.mark.parametrize("file_name", ESCAPING_NAMES, ids=str)
def test_a_file_name_cannot_reach_outside_the_attachments_directory(attachments_dir: Path, file_name: str) -> None:
...
def test_a_name_a_posix_filesystem_allows_is_still_accepted() -> None:
    """`:` and `*` are legal where these exports come from.

    Refusing them would report a file that is really there as bytes we do not have.
    """
    assert planning.safe_component("notes: draft *2.png")
    assert not planning.safe_component("notes\rdraft.png")


def test_a_symlink_is_recorded_at_the_target_that_was_checked(export_dir: Path, attachments_dir: Path) -> None:
    """`16` must upload the file `_within` validated, not whatever the link points at by then."""
...
def test_a_symlink_out_of_the_attachments_directory_is_refused(export_dir: Path, attachments_dir: Path) -> None:
...
def test_an_attachment_on_a_dropped_branch_is_not_planned(export_dir: Path, attachments_dir: Path) -> None:
    """Only the active path is migrated, so only its files are."""
    plan = plan_of(export_dir, attachments_dir)
    assert all(not item.attachments for item in plan.conversations if item.uuid != ATTACHMENT_CONVERSATION)
