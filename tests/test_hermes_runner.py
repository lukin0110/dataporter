...
def test_the_command_line_is_the_one_the_spec_names(
...
def test_a_run_id_that_is_not_a_filename_is_refused(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_a_fenced_object_after_chatter_validates(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_a_bare_object_validates_too(tmp_path: Path, fake: FakeHermes) -> None:
    fake.write(answer=json.dumps(RESULT))
    assert runner_for(tmp_path, fake).run("go", run_id="r", timeout_s=30).last_step == (
        "done"
    )


def test_a_helper_object_in_the_transcript_is_not_the_result(
    tmp_path: Path, fake: FakeHermes
) -> None:
    """Our own helpers print JSON and Hermes quotes it, so "the last object" is
    not a safe rule. The last object carrying `outcome` is."""
    fake.write(
        answer=(
            json.dumps(RESULT)
            + "\nThe helper said: "
            + json.dumps({"ok": True, "chars": 120, "sha256": "ab"})
            + "\n"
        )
    )
    result = runner_for(tmp_path, fake).run("go", run_id="r", timeout_s=30)
...
def test_the_last_result_wins_when_there_are_two(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_no_object_at_all_is_transient_and_names_the_file(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_an_object_that_is_not_a_result_is_still_no_result(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_a_result_with_a_bad_field_is_not_silently_replaced(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_unknown_fields_do_not_fail_a_good_run(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_every_needs_human_reason_the_spec_names_validates(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_a_rate_limited_result_carries_its_wait(
    tmp_path: Path, fake: FakeHermes
) -> None:
    fake.write(
        answer=json.dumps({**RESULT, "outcome": "rate_limited", "retry_after_s": 900})
    )
    assert runner_for(tmp_path, fake).run(
        "go", run_id="r", timeout_s=30
    ).retry_after_s == (900)
...
def test_another_non_zero_exit_with_a_valid_result_is_the_result(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_a_non_zero_exit_with_no_result_says_so(
    tmp_path: Path, fake: FakeHermes
) -> None:
...
def test_usage_is_read_from_the_file_hermes_was_given(
    tmp_path: Path, fake: FakeHermes
) -> None:
    fake.write(
...
def test_other_spellings_are_read_too(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {"prompt_tokens": 10, "completion_tokens": 2, "total_cost_usd": 1.5}
        ),
        encoding="utf-8",
    )
